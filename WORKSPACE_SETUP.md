...
O comando `init` configura tudo automaticamente de forma interativa: cria o `baseline.txt`, adiciona as entradas do pypeline ao `.gitignore` e verifica se as orgs estão autenticadas.
...
── pypeline init ───────────────────────────────────────────
  ...
  baseline.txt não encontrado. Criar agora com o commit HEAD atual? (y/n) y
✔  baseline.txt criado → a3f1b2c9...

  Entradas ausentes no .gitignore:
    - baseline.txt
    - build_deploy/
    - ...
  Adicionar entradas do pypeline ao .gitignore? (y/n) y
✔  Entradas adicionadas ao .gitignore.

Verificando orgs autenticadas...
✔  Org 'devops' autenticada.
✔  Org 'treino' autenticada.

────────────────────────────────────────────────────────────
✔  Workspace pronto. Execute sf pypeline run para iniciar o pipeline.
```

### Passo 2 — Simule o build (recomendado na primeira vez)
...
### Passo 3 — Execute o pipeline completo
...
### Passo 4 — Execute o quick deploy
