...
export default class PypelineDoctor extends SfCommand<PypelineDoctorResult> {
  ...
  public async run(): Promise<PypelineDoctorResult> {
    ...
    // ── 1. Git repository ─────────────────────────────────────────────────
    checks.push(this.checkGitRepo());

    // ── 2. Git status clean ───────────────────────────────────────────────
    checks.push(this.checkGitClean());

    // ── 3. SF CLI instalado ───────────────────────────────────────────────
    checks.push(this.checkSfCli());

    // ── 4. Node.js version ────────────────────────────────────────────────
    checks.push(this.checkNodeVersion());

    // ── 5. baseline.txt ───────────────────────────────────────────────────
    checks.push(this.checkBaseline());

    // ── 6. .pypeline.json ─────────────────────────────────────────────────
    checks.push(this.checkConfig());

    // ── 7. sfdx-project.json ──────────────────────────────────────────────
    checks.push(this.checkSfdxProject());
    ...
    checks.push(...this.checkOrgs(config));
  ...
  private checkGitClean(): CheckResult {
    try {
  ...
  private checkSfCli(): CheckResult {
    try {
  ...
  private checkBaseline(): CheckResult {
    ...
    // Verifica se o commit existe no repo
    try {
  ...
  private checkSfdxProject(): CheckResult {
  ...
  private checkOrgs(_config: Record<string, unknown>): CheckResult[] {
